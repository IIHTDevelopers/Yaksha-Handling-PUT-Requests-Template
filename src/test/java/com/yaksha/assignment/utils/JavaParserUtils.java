...
public class JavaParserUtils {
	...
	public static boolean checkControllerStructure(String filePath, String classAnnotations, String methodName,
		...
		try {
			...
			boolean hasClassAnnotation = compilationUnit.getClassByName("AppController").map(classDecl -> 
			...
			Optional<MethodDeclaration> methodOptional = compilationUnit.getClassByName("AppController")
					.flatMap(classDecl -> classDecl.getMethodsByName(methodName).stream().findFirst());

			if (methodOptional.isEmpty()) {
				System.out.println("Error: The method " + methodName + " was not found in the AppController class.");
				return false;
			}
			...
			boolean hasParamAnnotation = method.getParameterByName(paramName)
					.map(param -> param.getAnnotationByName("RequestBody").isPresent())
					.orElse(false);
